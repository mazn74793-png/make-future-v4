...
export default function ManageCoursesPage() {
  ...
  const [form, setForm] = useState({ title:'', description:'', thumbnail_url:'', price:0, duration:'', stage:'', is_published:false, is_free:false });

  const inp = "w-full bg-white/5 border border-white/10 rounded-xl py-3 px-4 text-white focus:border-purple-500 focus:outline-none";
  ...
  const handleSave = async () => {
    ...
    setEditing(null); setShowForm(false); loadCourses();
  };
  ...
  const handleDelete = async (id, title) => {
    ...
    if (videos?.length > 0) {
      await fetch('/api/delete-course-videos', { method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify({ keys: videos.map(v => v.r2_file_key) }) });
    }
    await supabase.from('courses').delete().eq('id', id);
    toast.success('اتمسح'); loadCourses();
  };

  const togglePublish = async (id, current) => {
    await supabase.from('courses').update({ is_published: !current }).eq('id', id);
    toast.success(current ? 'تم الاخفاء' : '✅ تم النشر'); loadCourses();
  };

  const startEdit = (course) => { setForm(course); setEditing(course.id); setShowForm(true); };
  const resetForm = () => { setForm({ title:'', description:'', thumbnail_url:'', price:0, duration:'', stage:'', is_published:false, is_free:false }); setEditing(null); setShowForm(!showForm); };

  return (
    <div>
      <div className="flex items-center justify-between mb-8">
        <h1 className="text-3xl font-black">ادارة الكورسات</h1>
        <button onClick={resetForm} className="gradient-primary px-6 py-3 rounded-xl text-white font-bold flex items-center gap-2">
      ...
      {showForm && (
        <div className="glass rounded-2xl p-6 mb-8 animate-fade-in space-y-4">
          ...
          <input type="text" value={form.title} onChange={e => setForm({...form, title:e.target.value})} placeholder="اسم الكورس *" className={inp} />
          <textarea value={form.description} onChange={e => setForm({...form, description:e.target.value})} placeholder="وصف الكورس" rows={3} className={`${inp} resize-none`} />

          {/* Stage */}
          <select value={form.stage} onChange={e => setForm({...form, stage:e.target.value})} className={inp}>
            <option value="">كل الصفوف (بدون تحديد)</option>
            {STAGES.filter(s => s).map(s => <option key={s} value={s}>{s}</option>)}
          </select>

          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <input type="text" value={form.thumbnail_url} onChange={e => setForm({...form, thumbnail_url:e.target.value})} placeholder="رابط الصورة" className={inp} />
            <input type="text" value={form.duration} onChange={e => setForm({...form, duration:e.target.value})} placeholder="المدة (مثال: 20 ساعة)" className={inp} />
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <input type="number" value={form.price} onChange={e => setForm({...form, price:e.target.value})} placeholder="السعر (جنيه)" className={inp} />
          ...
          <button onClick={handleSave} className="gradient-primary px-8 py-3 rounded-xl text-white font-bold">
        ...
        {courses.map((course, i) => (
          <div key={course.id} className="glass rounded-xl p-4 flex items-center gap-4 card-hover animate-fade-in" style={{ animationDelay:`${i*0.05}s` }}>
            {course.thumbnail_url
              ? <img src={course.thumbnail_url} alt="" className="w-20 h-14 rounded-lg object-cover flex-shrink-0" />
              : <div className="w-20 h-14 gradient-primary rounded-lg flex items-center justify-center text-2xl flex-shrink-0">📚</div>}
