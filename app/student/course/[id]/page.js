...
import { FiPlay, FiLock, FiYoutube } from 'react-icons/fi';
  ...
  useEffect(() => {
    const load = async () => {
      ...
      const { data: enrollment } = await supabase.from('enrollments')
        .select('id').eq('student_id', studentData.id).eq('course_id', id).eq('is_active', true).single();

      const access = !!enrollment || courseData.visibility === 'public';
      setHasAccess(access);

      if (access) {
        const { data: videosData } = await supabase.from('videos').select('*').eq('course_id', id).order('order');
  ...
  return (
    <div className="min-h-screen gradient-dark" dir="rtl">
      <div className="glass border-b border-white/5 px-6 py-4 flex items-center gap-4 sticky top-0 z-10">
        {/* زرار رجوع صح — مش router.back() */}
        <a href="/student" className="p-2 rounded-xl bg-white/5 hover:bg-white/10 transition text-white">
          ←
        </a>
        <div>
      ...
      {!hasAccess ? (
        <div className="flex items-center justify-center min-h-[80vh]">
          <div className="glass rounded-2xl p-10 text-center max-w-md">
            ...
            <a href="/student" className="gradient-primary px-8 py-3 rounded-xl text-white font-bold inline-block">رجوع</a>
          </div>
        </div>
      ) : (
              ...
              <>
                ...
                <div className="mt-4">
                  <div className="flex items-center gap-2 mb-1">
                    {activeVideo.video_type === 'youtube' ? <FiYoutube className="text-red-400" /> : <FiPlay className="text-purple-400" />}
                    <h2 className="text-xl font-black">{activeVideo.title}</h2>
                  </div>
                  {activeVideo.description && <p className="text-gray-400 text-sm mt-2">{activeVideo.description}</p>}
                </div>
              </>
            ) : (
                ...
                {videos.map((v, i) => (
                  <button key={v.id} onClick={() => setActiveVideo(v)}
                    className={`w-full text-right p-3 rounded-xl transition flex items-center gap-3 ${
                      activeVideo?.id === v.id ? 'gradient-primary text-white' : 'bg-white/5 hover:bg-white/10 text-gray-300'
                    }`}>
                    <div className={`w-8 h-8 rounded-lg flex items-center justify-center flex-shrink-0 text-sm font-bold ${
                      activeVideo?.id === v.id ? 'bg-white/20' : 'bg-white/10'
                    }`}>
                      {v.video_type === 'youtube' ? <FiYoutube className="text-sm" /> : i + 1}
                    </div>
