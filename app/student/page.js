            ...
            <div className="grid grid-cols-3 gap-3">
              {[
                ...
                <div key={i} className="glass rounded-2xl p-4 text-center">
                  <div className={`w-10 h-10 ${s.bg} rounded-xl flex items-center justify-center mx-auto mb-2 ${s.color}`}>
                    {s.icon}
                  </div>
                      ...
                      <div key={exam.id} className="glass rounded-2xl p-4 flex items-center justify-between border border-blue-400/10">
                        ...
                        {attempt?.is_submitted
                          ...
                          : <a href={`/student/exam/${exam.id}`}
                              className="gradient-primary px-4 py-2 rounded-xl text-white text-sm font-bold hover:opacity-90">
                              ابدأ
                            </a>}
                      </div>
                    );
        ...
        {activeTab === 'courses' && (
          <div className="space-y-8">
            {enrolledCourses.length > 0 && (
              <div>
                <h2 className="text-xl font-black mb-4 flex items-center gap-2">
                  <FiUnlock className="text-green-400" /> كورساتي المتاحة
                </h2>
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
                  {enrolledCourses.map(course => (
                    <a key={course.id} href={`/student/course/${course.id}`}
                      ...
                      {course.thumbnail_url
                        ? <img src={course.thumbnail_url} alt={course.title} className="w-full h-40 object-cover group-hover:opacity-90 transition" />
            ...
            {availableCourses.length > 0 && (
              <div>
                <h2 className="text-xl font-black mb-4 flex items-center gap-2">
                  <FiLock className="text-yellow-400" /> كورسات تحتاج إذن
                </h2>
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
                    ...
                    return (
                      <div key={course.id} className="glass rounded-2xl overflow-hidden border border-white/5 opacity-80">
                        ...
                        <div className="p-4">
                          ...
                          {reqStatus === 'pending' && (
                            <div className="w-full bg-yellow-400/10 text-yellow-400 py-2 rounded-xl text-sm text-center flex items-center justify-center gap-1">
                    ...
                    <div key={exam.id} className={`glass rounded-2xl p-5 border ${submitted ? (passed ? 'border-green-400/20' : 'border-red-400/20') : 'border-blue-400/20'}`}>
                      <div className="flex items-center justify-between flex-wrap gap-4">
                        <div>
                          ...
                          {exam.instructions && (
                            <p className="text-gray-500 text-xs mt-2 line-clamp-2">{exam.instructions}</p>
                          )}
                        </div>
                        <div className="flex items-center gap-3">
                          {submitted ? (
                            ...
                            <a href={`/student/exam/${exam.id}`}
                              className="gradient-primary px-6 py-3 rounded-xl text-white font-bold hover:opacity-90 transition flex items-center gap-2">
